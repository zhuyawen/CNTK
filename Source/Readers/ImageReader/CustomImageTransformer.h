...
namespace CNTK {
    ...
    public:
        explicit RandFloat() :
            mean_(0), std_(0), range_(0), max_inclusive_(true),
            is_uniform(range_ > 0 && std_ == 0)
        {
        }
    ...
    public:
        RandCrop() {}
        RandCrop(int crop_size, float offset_factor, float mean, float std, float range, bool adjust_if_out_of_boundary = true) :
            RandFloat(mean, std, range, true), crop_size(crop_size), offset_factor(offset_factor), adjust_if_out_of_boundary(adjust_if_out_of_boundary) {
        }
        ...
        inline float relative_pos(int img_size, std::mt19937 &rng) {
            ...
            if (intersect_min <= intersect_max) { //legal interval has intersect with desire interval
